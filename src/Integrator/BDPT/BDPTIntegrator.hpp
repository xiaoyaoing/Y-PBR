...
Spectrum
connectPath(const Scene& scene, const Camera* camera, const PathVertex* lightPath, int ln, const PathVertex* cameraPath, int cn, float* misWeight, ivec2* pRaster);

class BDPTIntegrator : public SamplerIntegrator {
public:
    BDPTIntegrator(std::shared_ptr<Camera> camera, std::shared_ptr<Sampler> sampler, const Json& json) : SamplerIntegrator(camera, sampler, json) {}
