...
void PhotonMapper::render(const Scene & scene) const {
    ...
    for ( int i = 0 ; i < pixelNum ; i ++ ) pixels[i].radius = initRadius;
    ...
    for ( int iteration = 0 ; iteration < iterations ; iteration ++ ) {
        parallel_for([&](ivec2 tileIndex) {
            ...
            tileSampler->setSeed(( tileIndex.y * nTiles.x + tileIndex.x ) * iteration);
                    ...
                    for ( int bounce = 0 ; bounce < maxBounces ; bounce ++ ) {
                        ...
                        if ( ! its ) {
                            for ( auto light: scene.lights )
                               ;// pixel.Ld += throughput * light->environmentLighting(ray);
                            break;
                        }
                        ...
                        if ( bounce < maxBounces - 1 ) {
                            Spectrum f = bsdf->sampleF(event, tileSampler->getNext2D());
                            if ( event.pdf == 0 || isBlack(f) ) break;
                            specularBounce = ( event.sampleType & BSDF_SPECULAR ) != 0;
                            throughput *= AbsCosTheta(event.wi) * f / event.pdf;
                            ...
                            ray = Ray(its->p, event.toWorld(event.wi), Constant::EPSILON);
                        }
                    }
                    if ( isDielectric ) {
                        dielectribVPNum ++;
        ...
        parallel_for([&](int photonIndex) {
            ...
            for ( int bounce = 0 ; bounce < 8; bounce ++ ) {
                ...
                if ( bounce > 0 ) {
                    ...
                    if ( gridBounds.Contains(photonP) ) {
                        ...
                        for ( SPPMListNode * node = atomic_load(&grids[gridIndex]) ;
                            ...
                            Spectrum contrib = event->its->bsdf->f(*event) * throughput;
                            if(hasNan(contrib)){
                ...
                Spectrum f = photonBsdf->sampleF(event, vec2(RadicalInverse(haltonDim + 0, haltonIndex),
                                                             RadicalInverse(haltonDim + 1, haltonIndex)));
                haltonDim += 2;
                Spectrum throughputNew = throughput * f * abs(event.wi.z) / event.pdf;
        ...
        parallel_for([&](int index) {
            ...
            if ( pixel.curPhotonCount > 0 ) {
                ...
                for ( int i = 0 ; i < 3 ; i ++ )
                    phi[i] = pixel.phi[i];
                pixel.flux = ( pixel.flux + phi * pixel.vp.throughPut  ) * ( newRadius * newRadius ) / ( pixel.radius * pixel.radius );
...
//                         photonDielectricNum, dielectribVPNum, photonDielectricBounce0Num);
            spdlog::info("{0} {1} {2} {3}", toColorStr(averageFlux.value),visiblePhotonCount,averageRadius,photonGridNum);
            visiblePhotonCount=0;
            photonGridNum = 0;
            averageFlux.reset();
            //count3 因为俄罗斯轮盘赌break的 notIntersectnum 因为没相交的
            visiblePhotonCount = 0;
            photonGridNum = 0;
            dielectricNum = 0;
