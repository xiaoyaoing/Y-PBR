...
int64_t HaltonSampler::GetIndexForSample(int64_t sampleNum) const {
    if (curPixel != pixelForOffset) {
        ...
        if (sampleStride > 1) {
            ...
            for (int i = 0; i < 2; ++i) {
                uint64_t dimOffset =
                    (i == 0) ? InverseRadicalInverse<2>(pm[i], baseExponents[i]) : InverseRadicalInverse<3>(pm[i], baseExponents[i]);
...
Float HaltonSampler::SampleDimension(int64_t index, int dimension) const {
    ...
    if (dimension == 0)
        ...
        return ScrambledRadicalInverse(dimension, index, PermutationForDimension(dimension));
}
