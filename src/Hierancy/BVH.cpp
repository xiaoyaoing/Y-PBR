...
struct BuildNode {
    ...
    std::optional<Intersection> intersect(const std::vector<std::shared_ptr<Primitive>>& primitives, Ray& ray) {
        IntersectionCount++;
...
BuildNode* BVHAccel::RecursiveBuild(std::vector<BVHPrimitiveInfo>& primitiveInfo, int start, int end, std::vector<std::shared_ptr<Primitive>>& orderedPrims) {
    if (start == end) {
        return nullptr;
    }
    BuildNodeCount++;
    ...
    switch (splitMethod) {
        case Middle: {
            ...
            BVHPrimitiveInfo* midPtr = std::partition(
                &primitiveInfo[start], &primitiveInfo[end - 1] + 1, [dim, pmid](const BVHPrimitiveInfo& pi) {
        ...
        case EqualCounts: {
            mid = (start + end) / 2;
            std::nth_element(&primitiveInfo[start], &primitiveInfo[mid], &primitiveInfo[end - 1] + 1, [dim](const BVHPrimitiveInfo& a, const BVHPrimitiveInfo& b) {
        ...
        case SAH: {
            ...
            std::sort(primitiveInfo.begin() + start, primitiveInfo.begin() + end, [dim](const BVHPrimitiveInfo& p1, const BVHPrimitiveInfo& p2) {
            ...
            for (int curBucket = 0; curBucket < bucketNum; curBucket++) {
                ...
                int splitPos           = std::lower_bound(primitiveInfo.begin() + start, primitiveInfo.begin() + end, tempInfo, [&dim](const BVHPrimitiveInfo& info, const BVHPrimitiveInfo& tempInfo) {
                                   return info.centroid[dim] < tempInfo.centroid[dim];
                               }) -
                               primitiveInfo.begin();

                Float splitCost = BVHPrimitiveInfo::computeSurfaceArea(primitiveInfo, start, splitPos) * (splitPos - start) + BVHPrimitiveInfo::computeSurfaceArea(primitiveInfo, splitPos, end) * (end - splitPos);
