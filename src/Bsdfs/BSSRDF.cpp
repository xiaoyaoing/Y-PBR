...
Float SampleCatmullRom2D(int size1, int size2, const Float* nodes1, const Float* nodes2, const Float* values, const Float* cdf, Float alpha, Float u, Float* fval = nullptr, Float* pdf = nullptr) {
...
Spectrum SeparableBSSRDF::sampleS(const Scene& scene, Float u1, vec2 u2, const SurfaceEvent& po, Intersection* pi, Float* pdf) const {
...
Spectrum
SeparableBSSRDF::sampleSp(const Scene& scene, Float u1, const vec2& u2, const SurfaceEvent& po, Intersection* pi, Float* pdf) const {
    po.frame.tangent;
    vec3 ns = po.frame.n;
...
Spectrum TabelBSSRDF::Sr(Float d, const SurfaceEvent& event) const {
    ...
    for (int ch = 0; ch < 3; ++ch) {
        ...
        if (!CatmullRomWeights(table.nRhoSamples, table.rhoSamples.get(), rho[ch], &rhoOffset, rhoWeights) ||
            !CatmullRomWeights(table.nRadiusSamples, table.radiusSamples.get(), rOptical, &radiusOffset, radiusWeights))
            continue;
...
Float TabelBSSRDF::sampleSr(int ch, Float u, const SurfaceEvent& event) const {
    ...
    auto t = SampleCatmullRom2D(table.nRhoSamples, table.nRadiusSamples, table.rhoSamples.get(), table.radiusSamples.get(), table.profile.get(), table.profileCDF.get(), rho[ch], u) /
             sigma_t[ch];
    return t;
}
...
TabelBSSRDF::TabelBSSRDF(Float eta, std::shared_ptr<Texture<Spectrum>> sigma_a, std::shared_ptr<Texture<Spectrum>> sigma_s, Float g)
...
Float BeamDiffusionMultScattering(Float sigmaS, Float sigmaA, Float g, Float eta, Float r) {
    ...
    for (int i = 0; i < nSamples; i++) {
        ...
        Float phiD  = Constant::INV_FOUR_PI / Dg * (std::exp(-sigmaTr * dr) / dr - std::exp(-sigmaTr * dv) / dv);
...
void computeBeamDiffusionBSSRDF(Float g, Float eta, BSSRDFTable* t) {
    ...
    parallel_for([&](int i) {
        ...
        for (int j = 0; j < t->nRadiusSamples; ++j) {
            ...
            auto  temp = 2 * Constant::PI * r * (BeamDiffusionMultScattering(rho, 1 - rho, g, eta, r) + BeamDiffusionSingleScattering(rho, 1 - rho, g, eta, r));
            if (isnan(temp)) {
            }
        ...
        // sampling
        t->rhoEff[i] =
            IntegrateCatmullRom(t->nRadiusSamples, t->radiusSamples.get(), &t->profile[i * t->nRadiusSamples], &t->profileCDF[i * t->nRadiusSamples]);
    },
                 t->nRhoSamples);
}
