...
namespace BSDFFactory {
    ...
    std::shared_ptr<Material> LoadRoughConductorMaterial(const Json& j) {
        ...
        auto        roughCounductorMaterial = std::make_shared<RoughConductor>(eta, k, LoadMicrofacetDistribution(distribStr), std::get<0>(roughnessTuple), std::get<1>(roughnessTuple), std::get<2>(roughnessTuple));
        if (contains(j, "material"))
    ...
    std::shared_ptr<Material> LoadRoughDielectricMaterial(const Json& j) {
        ...
        std::string distribStr              = getOptional(j, "distribution", std::string("beckmann"));
        auto        roughnessTuple          = loadRoughness(j);
        auto        roughDielectricMaterial = std::make_shared<RoughDielectric>(ior, LoadMicrofacetDistribution(distribStr), std::get<0>(roughnessTuple), std::get<1>(roughnessTuple), std::get<2>(roughnessTuple));
        ...
        for (auto& bssrdfJson : j) {
            {
                ...
                if (type == "disney") {
                    ...
                    sigmaS->setScale(scale);
                    sigmaA->setScale(scale);
