...
class BSSRDF {
public:
    virtual Spectrum sampleS(const Scene& scene, Float u1, vec2 u2, const SurfaceEvent& po, Intersection* pi, Float* pdf) const = 0;
...
public:
    ...
    Spectrum sampleS(const Scene& scene, Float u1, vec2 u2, const SurfaceEvent& po, Intersection* pi, Float* pdf) const override;
    Spectrum sampleSp(const Scene& scene, Float u1, const vec2& u2, const SurfaceEvent& po, Intersection* pi, Float* pdf) const;
...
public:
    DisneyBSSRDF(std::shared_ptr<Texture<Spectrum>> scatterDistance,
                 ...
                 Float                              eta) : SeparableBSSRDF(eta), scatterDistance(scatterDistance), color(color) {}
};
...
public:
    ...
    TabelBSSRDF(Float eta, std::shared_ptr<Texture<Spectrum>> sigma_a, std::shared_ptr<Texture<Spectrum>> sigma_s, Float g);
