...
Spectrum Hair::f(const SurfaceEvent& event) const {
    ...
    Float h = getH(event);
    if (useGussianAmz) {
        return MR * _nR->eval(phi, cosThetaD) + MTT * _nTT->eval(phi, cosThetaD) + MTRT * _nTRT->eval(phi, cosThetaD);
    }
...
Float Hair::Pdf(const SurfaceEvent& event) const {
    ...
    if (useGussianAmz) {
        ...
        return (1.0f / weightSum) *
               (pdfR * _nR->pdf(phi, cosThetaD) + pdfTT * _nTT->pdf(phi, cosThetaD) + pdfTRT * _nTRT->pdf(phi, cosThetaD));
    }
...
std::array<Float, pMax + 1> Hair::ComputeApPdf(Float cosThetaO, Float h) const {
    ...
    Float                       sumY =
        std::accumulate(ap.begin(), ap.end() - 1, Float(0), [](Float s, const Spectrum& ap) { return s + luminace(ap); });
