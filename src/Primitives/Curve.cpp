   ...
   // return std::nullopt;
    if(useEmbree) {
        ...
        if ( rayHit.hit.geomID != RTC_INVALID_GEOMETRY_ID ) {
            ...
            //t = 0.5;
            //p0 = 50;
            auto x = (its.p - vec3(_nodeData[p0])/vec3(_nodeData[p0+3]-_nodeData[p0]));
            auto y = (its.p - vec3(_nodeData[p0+1])/vec3(_nodeData[p0+3]-_nodeData[p0]));
            auto z = (its.p - vec3(_nodeData[p0+2])/vec3(_nodeData[p0+3]-_nodeData[p0]));
            auto w = (its.p - vec3(_nodeData[p0+3])/vec3(_nodeData[p0+3]-_nodeData[p0]));

            vec3 tangent = normalize(BSpline::quadraticDeriv(_nodeData[p0], _nodeData[p0 + 1], _nodeData[p0 + 2],_nodeData[p0+3], t));
