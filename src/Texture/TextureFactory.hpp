...
namespace  TextureFactory{
    template <class T>
    std::shared_ptr<Texture<T>> LoadTexture(const nlohmann::json & texturJson,const T defaultValue){
        ...
        if(texturJson.is_object()){
            ...
            if(textureType == "checker"){
                T onColor= getOptional(texturJson,"on_color",defaultValue);
                T offColor= getOptional(texturJson,"off_color",defaultValue);
